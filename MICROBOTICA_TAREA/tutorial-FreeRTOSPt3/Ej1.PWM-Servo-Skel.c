...
#define MOTOR_IZQ  0x02  //(1<<1)// 0x0002

//#define RECTO_3A
//#define GIRO_VARIABLE_3B
#define GIRO_SOBRE_SI_MISMO_3C
...
uint32_t DutyCycle_mitadvelocidad_der = (COUNT_1MS+STOPCOUNT2)/2; //
 ...
 static portTASK_FUNCTION(MOTORTask,pvParameters)
 {
     ...
     while(1)
     {
         eventosMOTOR = xEventGroupWaitBits(FlagsEventos,MOTOR_DER|MOTOR_IZQ,pdFALSE,pdFALSE,portMAX_DELAY);
         ...
         if (eventosMOTOR & MOTOR_DER)
         {
             ...
             //3.a
                    #ifdef RECTO_3A
                     PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6, COUNT_2MS);//Esto es para que vaya recto
                     PWMPulseWidthSet(PWM1_BASE, PWM_OUT_7, COUNT_1MS);
                    #endif
             //3.c
                    #ifdef GIRO_SOBRE_SI_MISMO_3C
                     PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6, COUNT_2MS);
                     PWMPulseWidthSet(PWM1_BASE, PWM_OUT_7, COUNT_2MS);//Esto es para que gire sobre
                     #endif
             //ej3.b
            #ifdef GIRO_VARIABLE_3B
             if(DutyCycle_mitadvelocidad_der <= COUNT_1MS)
             {
                 DutyCycle_mitadvelocidad_der = COUNT_1MS;
             }
             else
             {
                 DutyCycle_mitadvelocidad_der -= CYCLE_INCREMENTS;
         ...
         {
             xEventGroupClearBits(FlagsEventos,MOTOR_IZQ);
             //3.a
                    #ifdef RECTO_3A
                     PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6, COUNT_1MS);//Esto es para que vaya recto
             ...
             //ej3.b
            #ifdef GIRO_VARIABLE_3B
             if(DutyCycle_mitadvelocidad_der >= STOPCOUNT2)
             {
                 DutyCycle_mitadvelocidad_der = STOPCOUNT2;
             }
             else
             {
                 DutyCycle_mitadvelocidad_der += CYCLE_INCREMENTS;
...
void GPIOFIntHandler(void)
{
    ...
    if(((i32Status & RIGHT_BUTTON) == RIGHT_BUTTON)){
        ...
        xEventGroupSetBitsFromISR(FlagsEventos, MOTOR_IZQ,higherPriorityTaskWoken);
