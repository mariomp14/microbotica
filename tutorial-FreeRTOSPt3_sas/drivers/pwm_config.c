...
void config_pwm_motors(void)
{
    SysCtlPWMClockSet(SYSCTL_PWMDIV_64);    // Establece reloj del sistema (40MHz/64=625KHz)
        ...
        GPIOPinTypePWM(GPIO_PORTC_BASE, GPIO_PIN_4);    // PB6 como salida PWM
        GPIOPinConfigure(GPIO_PC4_M0PWM6);     // del módulo PWM1 (ver tabla 10.2 Data-Sheet, columnas 4 y 5)
        ...
        PWMGenConfigure(PWM0_BASE, PWM_GEN_3, PWM_GEN_MODE_DOWN);   // Módulo PWM contara hacia abajo
        PWMGenPeriodSet(PWM0_BASE, PWM_GEN_3, PERIOD_PWM); //Aunque ponga 32 bits, lo maximo que permite son 16 bits.

        //PWMPulseWidthSet(PWM1_BASE, PWM_OUT_7, STOPCOUNT2);Esto es para que empiece el programa con el motor izquierdo parado
        PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6, STOPCOUNT); //DERECHO // Establece el ciclo de trabajo (en este caso, un porcentaje del valor máximo, el periodo)
        PWMOutputState(PWM1_BASE, PWM_OUT_6_BIT, true);

        //PWMPulseWidthSet(PWM1_BASE, PWM_OUT_6, STOPCOUNT); Esto es para que empiece el programa con el motor derecho parado
        PWMPulseWidthSet(PWM1_BASE, PWM_OUT_7, STOPCOUNT2); //IZQUIERDO // Establece el ciclo de trabajo (en este caso, un porcentaje del valor máximo, el periodo)
        PWMOutputState(PWM1_BASE, PWM_OUT_7_BIT, true);// Habilita la salida de la señal

        PWMPulseWidthSet(PWM0_BASE, PWM_OUT_6, STOPCOUNT);  // Establece el ciclo de trabajo (en este caso, un porcentaje del valor máximo, el periodo)
        PWMOutputState(PWM0_BASE, PWM_OUT_6_BIT, true);// Habilita la salida de la señal
